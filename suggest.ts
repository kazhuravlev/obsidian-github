...
export class DirSuggest extends TextInputSuggest<TFolder> {
  getSuggestions(inputStr: string): TFolder[] {
    const abstractFiles = this.app.vault.getAllLoadedFiles();
    const dirs: TFolder[] = [];
    const lowerCaseInputStr = inputStr.toLowerCase();

    abstractFiles.forEach((dir: TAbstractFile) => {
      if (dir instanceof TFolder && dir.path.toLowerCase().contains(lowerCaseInputStr)) {
        dirs.push(dir);
      }
    });
